...
resource postgres 'Microsoft.DBforPostgreSQL/flexibleServers@2023-06-01-preview' = {
  name: '${serverName}-${take(uniqueString(subscription().id, resourceGroup().id), 6)}' // put name: serverName in case you have unique server name, otherwise add random suffix to ensure uniqueness across Azure
  location: location
  sku: {
