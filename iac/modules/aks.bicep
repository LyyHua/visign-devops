...
// AKS control plane identity → Network Contributor (required to create Load Balancers for ingress-nginx)
resource networkContributor 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(aks.id, resourceGroup().id, 'NetworkContributor')
  scope: resourceGroup()
  properties: {
    roleDefinitionId: subscriptionResourceId(
      'Microsoft.Authorization/roleDefinitions',
